...
# Serializes the check-then-act sections of booking creation and cancellation so
# conflict/quota/cancel decisions hold correctly under concurrent requests.
...
def _has_conflict(db: Session, room_id: int, start: datetime, end: datetime) -> bool:
    ...
    _pricing_warmup()
    for b in existing:
        # Strict inequalities: back-to-back bookings (one ends exactly when the
...
def create_booking(
    ...
    # Serialize conflict + quota checks with the insert so they hold under
    # concurrent requests (a bare check-then-act would let both winners commit).
    with _booking_lock:
...
def cancel_booking(
    ...
    # Serialize the cancel state transition so concurrent cancels of the same
    # booking produce exactly one RefundLog entry and one refund.
    with _booking_lock:
        db.refresh(booking)
